    ...
    >
      <div style="height: 90%;overflow: auto">
        ...
        <el-row style="text-align: left">
          ...
          <el-select
            @change="receiveAddressIdChange(receiveAddressId)"
            v-model="receiveAddressId"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-table
          ...
          style="width: 100%"
        >
          <el-table-column prop="mainPicture" label="商品图片" width="200px">
          ...
          <el-table-column prop="productAttrShow" label="规格">
          </el-table-column>
          ...
          handle: () => {
            if (this.selection.length <= 0) {
              ...
              product.getReceiveAddressDropDown().then(res => {
                ...
                if (
                  this.receiveAddressDropDown &&
                  this.receiveAddressDropDown.length > 0
                ) {
                  // 选中的地址
              ...
              for (let i = 0; i < cartItemListMid.length; i++) {
                cartItemListMid[i].totalPrice =
                  cartItemListMid[i].quantity * cartItemListMid[i].price;
              }
      ...
      product.getCartItemList(this.searchData).then(res => {
        ...
        if (records && records.length > 0) {
          for (let i = 0; i < records.length; i++) {
            ...
            records[i].mainPicture = resolvePicFileRes(
              records[i].mainPictureFile
            ).url;
          }
        }
