...
import {
  ...
  updateOrderNote,
  deleteOrder
} from "@/http/implement/order";
import LogisticsDialog from "@/pages/orderManager/components/logisticsDialog";
  ...
  created() {
    ...
    getOrderDetail({id:this.id}).then(res => {
      ...
      for (let i=0;i<orderItemList.length;i++) {
        ...
        orderItem.productPic = resolvePicFileRes(orderItem.shopFileResp).url;
      }
      this.order = res.data;
    });
  },
  ...
  methods: {
    ...
    handleUpdateReceiverInfo() {
      this.$confirm("是否要修改收货信息?", "提示", {
        ...
        updateReceiverInfo(this.receiverInfo).then(res => {
    ...
    handleUpdateMoneyInfo() {
      this.$confirm("是否要修改费用信息?", "提示", {
        ...
        updateMoneyInfo(this.moneyInfo).then(res => {
